...
function cargarDatosAsistencia(orden) {
    ...
    fetch('data/asistencia.json')
        ...
        .then(datos => {
            ...
            faltas_restantes = datos['Faltas_restantes'];  // Obtiene las faltas restantes de los datos

            // Ordena los datos según el orden especificado
            let datosOrdenados;
            switch (orden) {
                case 'porcentaje':
                    datosOrdenados = porcentaje;
                    break;
                case 'faltas':
                    datosOrdenados = faltas;
                    break;
                case 'faltas_restantes':
                    datosOrdenados = faltas_restantes;
                    break;
                default:
                    datosOrdenados = faltas;
            }
            ...
            for (let [asignatura, valor] of datosOrdenados) {
                ...
                let celda4 = fila.insertCell(3);  // Inserta una nueva celda en la fila
                ...
                celda4.innerHTML = faltas_restantes[asignatura];  // Asigna las faltas restantes a la cuarta celda
            }
        });
}
